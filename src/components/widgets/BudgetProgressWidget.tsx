...
export default function BudgetProgressWidget({ size = 'small' }: BudgetProgressWidgetProps) {
    ...
    switch (size) {
      case 'small':
        return {
          ...
          titleSize: 'text-base',
          emptyTextSize: 'text-sm',
          emptyIconSize: 24 as const,
          gapY: 'space-y-2',
        };
      case 'medium':
        return {
          showIcon: true,
          showOverBudgetBadge: true,
      ...
      case 'large':
        return {
          ...
          progressBarHeight: 'h-3',
          titleSize: 'text-base',
          emptyTextSize: 'text-base',
          emptyIconSize: 32 as const,
          gapY: 'space-y-4',
        };
      default:
        return {
  ...
  if (progress.budget <= 0) {
    return (
      ...
      >
        <div className={`${sizeConfig.iconContainerSize} bg-primary-50 dark:bg-primary-900/20 rounded-xl flex items-center justify-center mb-3`}>
          <Target size={sizeConfig.emptyIconSize} className="text-primary-500" />
        </div>
        <p className={`font-medium ${sizeConfig.emptyTextSize}`}>设置年度预算</p>
    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          {sizeConfig.showIcon && (
            <div className={`${sizeConfig.iconContainerSize} rounded-lg flex items-center justify-center ${
              progress.isOverBudget 
                ? 'bg-red-50 dark:bg-red-900/20' 
                : 'bg-primary-50 dark:bg-primary-900/20'
            }`}>
              <Target size={sizeConfig.iconSize} className={progress.isOverBudget ? 'text-red-500' : 'text-primary-500'} />
            </div>
          )}
          <span className={`font-medium text-ink-800 dark:text-ink-200 ${sizeConfig.titleSize}`}>
            {currentYear}年度预算
          </span>
        </div>
        {sizeConfig.showOverBudgetBadge && progress.isOverBudget && (
      ...
      <div className={`flex-1 ${sizeConfig.gapY}`}>
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <span className={`text-ink-500 dark:text-ink-400 ${size === 'large' ? 'text-sm' : 'text-xs'}`}>已使用 {progress.percentage.toFixed(0)}%</span>
            <span className={`${getTextColor(progress.percentage)} tabular-nums ${
              size === 'large' ? 'text-base' : 'text-sm'
            } font-semibold`}>
              {formatMoney(progress.used, showCents)}
            </span>
          </div>
          <div className={`${sizeConfig.progressBarHeight} bg-cream-200 dark:bg-ink-700 rounded-full overflow-hidden`}>
            <div
        ...
        {sizeConfig.showBudgetBreakdown && (
        ...
        {sizeConfig.showMonthlyProgress && (
          <div className={`pt-2 border-t border-ink-100 dark:border-ink-700 ${size === 'small' ? 'hidden' : ''}`}>
            ...
            <div className={`${size === 'large' ? 'h-2' : 'h-1.5'} bg-cream-200 dark:bg-ink-700 rounded-full overflow-hidden mt-1`}>
              <div
